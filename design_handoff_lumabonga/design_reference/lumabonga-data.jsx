...
function useLumaStore(seed) {
  const [products, setProducts] = React.useState(seed?.products || SEED_PRODUCTS);
  const [sales, setSales] = React.useState(seed?.sales || SEED_SALES);
  const [purchases, setPurchases] = React.useState(seed?.purchases || SEED_PURCHASES);
  const [costs, setCosts] = React.useState(seed?.costs || SEED_COSTS);
  const [production, setProduction] = React.useState(seed?.production || SEED_PRODUCTION);
  const [activeUser, setActiveUser] = React.useState('lumaya'); // 'lumaya' | 'gawah'
  const [materials, setMaterials] = React.useState(() =>
    (seed?.materials || SEED_MATERIALS).map((m) => ({ stock0: STOCK0_BY_UNIT[m.unit] ?? 0, ...m })));
  ...
  const [recipes, setRecipes] = React.useState(() => withIds(seed?.recipes || SEED_RECIPES));
